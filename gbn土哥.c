...
/* ******************************************************************
   ...
   ALTERNATING BIT AND GO-BACK-N NETWORK EMULATOR: VERaSION 1.1  J.F.Kurose
...
static struct pkt buffer[WINDOWSIZE+1];  /* array for storing packets waiting for ACK */
...
void A_output(struct msg message)
{
  ...
  if ( windowcount < WINDOWSIZE) {
    ...
    A_nextseqnum = (A_nextseqnum + 1) % 7;  /* we only have seqnum 0 and 1 */
  }
...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    /* check if new ACK or duplic(ate */
   if (windowcount>0&& buffer[windowlast].seqnum >=packet.acknum &&buffer[windowfirst].seqnum<=packet.acknum) {    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      /* packet is a new ACK */
...
void A_timerinterrupt(void)
{
  ...
  if (TRACE > 0)
    ...
    for(at=windowfirst;at<=windowlast;at++)
    {
  if (TRACE > 0)
    printf ("---A: resending packet %d\n", buffer[at].seqnum);
        tolayer3(A,buffer[at]);
        if(at==windowfirst){
        starttimer(A,RTT);
        }
    }
...
void B_input(struct pkt packet)
{
  ...
  if  ( (!IsCorrupted(packet))  && (packet.seqnum == expectedseqnum) ) {
    ...
    expectedseqnum = (expectedseqnum + 1) % 7;
  }
  else {
    ...
    sendpkt.acknum=(expectedseqnum-1+7)%7;
  }
